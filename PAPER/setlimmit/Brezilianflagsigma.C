...
sigma[idx]=(sfactor/lumi)*(X[idx]/eff);
cout<<sfactor<<"       "<<sfactor/lumi<<"          "<<X[idx]/eff<<"             "<<(sfactor/lumi)*(X[idx]/eff)<<endl;
}
...
//   grafexp1sigma->
   ...
   grafexp2sigma->GetXaxis()->SetRangeUser(0, sigma[4]+0.1*sigma[4]);
   ...
   grafexp2sigma->GetXaxis()->SetTitle("#sigma_{signal}(pb)");
   grafexp2sigma->SetTitle("");

   TGraph *Expected= new TGraph(N,sigma,nE);
   ...
   TGraph *Observed= new TGraph(N,sigma,nO);
...
//   Observed->SetLineStyle(2);
   ...
   Observed->Draw("CP");
    ...
    TLine *line1 = new TLine(sigma[0], 1, sigma[4], 1);
