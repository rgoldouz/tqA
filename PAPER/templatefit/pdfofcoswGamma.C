...
Coswphoton= new TH1F("Coswphoton","Coswphoton",22 ,-1 ,1);
...
else {if(samples_[idx]=="WPHJET.root") Coswphoton->Fill(coswph ,weight);}
}
}}
...
convpui->cd(0);
wphjethists->SetLineColor(kGreen+1);
wphjethists->SetMaximum(0.15);
wphjethists->SetMinimum(0);
wphjethists->SetLineWidth(3);
wphjethists->GetXaxis()->SetTitle("cos(w,#gamma )");
wphjethists->Draw();
BGSR[samples_.size()-1]->SetLineColor(2);
