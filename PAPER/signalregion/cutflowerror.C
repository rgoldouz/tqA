...
for(unsigned int idx=0; idx<samples_.size(); ++idx){
hists[idx]->Scale(sqrt(hists[idx]->Integral())/hists[idx]->Integral());
hists[idx]->Scale(lumi*scales[idx]);
}
for(unsigned int idx=0; idx<samples_.size(); ++idx){
cout<<samples_[idx]<<"              "<<hists[idx]->Integral()<<endl;
}
cout<<"total single top"<<"              "<<sqrt(pow(hists[4]->Integral(),2)+pow(hists[5]->Integral(),2)+pow(hists[8]->Integral(),2)+pow(hists[9]->Integral(),2))<<endl;

//pow(hists[6]->Integral(),2)+pow(hists[7]->Integral(),2)+
cout<<"total ttbar"<<"              "<<hists[10]->Integral()+hists[11]->Integral()+hists[12]->Integral()<<endl;
...
double error=0;
for(unsigned int idx=0; idx<samples_.size(); ++idx){
if (idx!=2 && idx!=7) error=error+pow(hists[idx]->Integral(),2);
}

cout<<"$$$$$$$$$ total statistical error $$$$$$$       =   "<<sqrt(error+pow(75.06,2)+pow(138.8,2))<<endl;
