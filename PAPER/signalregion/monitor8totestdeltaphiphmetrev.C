...
void monitor8totestdeltaphiphmetrev() {
...
for(unsigned int idx=0; idx<datasamples_.size(); ++idx){
	...
	for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
	theTree->GetEntry(ievt);
	if ((*mymasstop )[0]>130 && (*mymasstop )[0]<220 && (*myetaphoton)[0]>1.444 ) {
...
double nwjet=(219.373/hists_muoneta[samples_.size()+5]->Integral())*(insidewjet/(insidewjet+outsidewjet));
...
*/
double nwphjet=(1112.2/hists_muoneta[8]->Integral())*(insidewphjet/(insidewphjet+outsidewphjet));
