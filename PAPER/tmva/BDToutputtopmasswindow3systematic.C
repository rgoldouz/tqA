...
using namespace std;
void BDToutputtopmasswindow3systematic() 
{   
#ifdef __CINT__
...
cout<<"__JES__plus         "<<jesuphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__JES__minus         "<<jesdownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__JER__plus         "<<jeruphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__JER__minus         "<<jerdownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__PhES__plus         "<<phesuphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__PhES__minus         "<<phesdownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__PU__plus         "<<puuphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__PU__minus         "<<pudownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__TRIG__plus         "<<triguphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__TRIG__minus         "<<trigdownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;

std::vector<TH1F*> btaguphists;
for(unsigned int i=0; i<variables_.size(); ++i){
name=variables_[i]+"__btagSF__plus";
...
cout<<"__btagSF__plus         "<<btaguphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;

std::vector<TH1F*> btagdownhists;
for(unsigned int i=0; i<variables_.size(); ++i){
name=variables_[i]+"__btagSF__minus";
...
cout<<"__btagSF__minus         "<<btagdownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
/*
...
*/
//cout<<"rrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr1"<<endl;
...
cout<<"__MUON__plus         "<<muonuphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__MUON__minus         "<<muondownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__PHOTON__plus         "<<photonuphists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;
...
cout<<"__PHOTON__minus         "<<photondownhists[variables_.size()-1]->Integral()+wjetandwphjet[0]->Integral()+wjetandwphjet[1]->Integral()<<endl;

std::vector<std::vector<double_t> > vec(photondownhists[0]->GetNbinsX(), vector<double>(16));
  for(int p = 0; p <photondownhists[0]->GetNbinsX(); p++){ //loop over bins
    for(int m = 0; m < 16; m++){ //loop over systematics
...
float scales[] ={0.628,0.0978,34.01,6.133,1.04,0.32,0.02,0.002,0.0961,0.0253,0.0224,0.0145,0.0125,0.0160,0.0158,0.0341,0.0341,0.0341,0.020,0.0017,0.0055,0.0032,0.00084,0.02,0.01139,0.01139,0.061094905/19.145};
...
std::vector<std::vector<double_t> > vecplus(photondownhists[0]->GetNbinsX(), vector<double>(16));
std::vector<std::vector<double_t> > vecminus(photondownhists[0]->GetNbinsX(), vector<double>(16));
 for(int p = 0; p <photondownhists[0]->GetNbinsX(); p++){ //loop over bins
    for(int m = 0; m < 16; m++){ //loop over systematics
vecplus[p][m]=0;
vecminus[p][m]=0;
...
TPad *pad1 = new TPad("pad1","pad1",0,0.21,1,1);
pad1->SetFillStyle(0);
pad1->SetFrameFillStyle(0);
pad1->SetBottomMargin(0.022);
pad1->SetLeftMargin(0.115);
TPad *pad2 = new TPad("pad2","pad2",0,0.0,1,0.21);
pad2->SetFillStyle(0);
...
hs1->GetYaxis()->SetTitleSize(0.062*1.25);
...
errorup[p]=pow(0.33*wjetandwphjet[0]->GetBinContent(p+1),2)+errorup[p];
errordown[p]=pow(0.33*wjetandwphjet[0]->GetBinContent(p+1),2)+errordown[p];
errorup[p]=pow(0.25*wjetandwphjet[1]->GetBinContent(p+1),2)+errorup[p];
errordown[p]=pow(0.25*wjetandwphjet[1]->GetBinContent(p+1),2)+errordown[p];
...
//cout<<errordown[p]<<endl;
 ...
 double ax2[photondownhists[0]->GetNbinsX()];
 double ay2[photondownhists[0]->GetNbinsX()];
 double aexl2[photondownhists[0]->GetNbinsX()];
 double aexh2[photondownhists[0]->GetNbinsX()];
 double aeyl2[photondownhists[0]->GetNbinsX()];
 double aeyh2[photondownhists[0]->GetNbinsX()];
...
aexl[p]=0.05;
aexh[p]=0.05;
aeyl[p]=sqrt(errordown[p]+pow(sum_h->GetBinError(p+1),2));
aeyh[p]=sqrt(errorup[p]+pow(sum_h->GetBinError(p+1),2));
}

 for(int p = 0; p <photondownhists[0]->GetNbinsX(); p++){ //loop over bins
ax2[p]=-0.55+0.1*p;
ay2[p]=1;
//ay2[p]=datahists[2]->GetBinContent(p+1)/sum_h->GetBinContent(p+1);
aexl2[p]=0.05;
aexh2[p]=0.05;
if(sum_h->GetBinContent(p+1)!=0){
aeyl2[p]=sqrt(errordown[p]+pow(sum_h->GetBinError(p+1),2))/sum_h->GetBinContent(p+1);
aeyh2[p]=sqrt(errorup[p]+pow(sum_h->GetBinError(p+1),2))/sum_h->GetBinContent(p+1);}
else {
aeyl2[p]=0;
aeyh2[p]=0;
}
//aeyl2[p]=1-(sum_h->GetBinContent(p+1)-sqrt(errordown[p]+pow(sum_h->GetBinError(p+1),2)))/(sum_h->GetBinContent(p+1));
//aeyh2[p]=(sum_h->GetBinContent(p+1)+sqrt(errorup[p]+pow(sum_h->GetBinError(p+1),2)))/(sum_h->GetBinContent(p+1))-1;
}
...
TGraphAsymmErrors* gae2 = new TGraphAsymmErrors(photondownhists[0]->GetNbinsX(), ax2, ay2, aexl2, aexh2, aeyl2, aeyh2);
gae2->SetFillColor(1);
gae2->SetFillStyle(3003);
...
//  leg->AddEntry(gae, "Uncertainties (Syst#oplus Stat)"               , "F");
...
TH1F *newMC;
newMC = new TH1F( "newMC",           "newMC",           nbin, min, max );
for(int p = 0; p <photondownhists[0]->GetNbinsX(); p++){ //loop over bins
newMC->SetBinContent(p+1,sum_h->GetBinContent(p+1));}

h_ratio->Divide(newMC);
h_ratio->SetFillStyle(3004);
h_ratio->GetXaxis()->SetTitle("BDT output for tc#gamma");
h_ratio->GetYaxis()->SetTitle("DATA/MC");
  h_ratio->GetXaxis()->SetTitleSize(0.31);
  ...
  h_ratio->GetXaxis()->SetLabelSize(0.23);
  h_ratio->GetYaxis()->SetLabelSize(0.21);
  h_ratio->GetYaxis()->SetTitleOffset(0.24);
h_ratio->GetXaxis()->SetTitleOffset(0.68);
...
TCanvas *ct = new TCanvas("ct","signal region",50,50,865,780);
ct->cd();
hists[23]->Draw();
