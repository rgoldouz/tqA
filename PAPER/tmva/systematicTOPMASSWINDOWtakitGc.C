...
void systematicTOPMASSWINDOWtakitGc( TString myMethodList = "" ) 
{   
#ifdef __CINT__
...
float scales[] = {0.0978,1.491,0.0961,0.0253,0.0224,0.0145,0.0125,0.0160,0.0158,0.0341,0.0341,0.0341,0.020,0.0017,0.0055,0.0032,0.00084,0.02,0.01139,0.01139,0.049094905,0.049094905,0.049094905,0.049094905,0.0612/19.145};
...
for(unsigned int idx=0; idx<samples_.size(); ++idx){
   TFile *input(0);
TString fname;
if (phi<8) fname =systematics[phi]+"/"+samples_[idx];
...
float correctbtag;
if (phi==6) correctbtag=(float) Bmodification((*mycvsdiscriminant)[0],(*myjetmatchinginfo)[0],1);
else if (phi==7) correctbtag=(float) Bmodification((*mycvsdiscriminant)[0],(*myjetmatchinginfo)[0],-1);
else correctbtag=(float) Bmodification((*mycvsdiscriminant)[0],(*myjetmatchinginfo)[0],0);
cvsdiscriminant=correctbtag;
...
if (!(samples_[idx]=="TTBAR1.root" || samples_[idx]=="TTBAR2.root" || samples_[idx]=="SINGLE-TOP-PH.root" || samples_[idx]=="SIGNALtGC.root" || samples_[idx]=="WPHJET.root"||samples_[idx]=="T-W-CH.root" ||samples_[idx]=="TBAR-W-CH.root" ||samples_[idx]=="T-S-CH.root" ||samples_[idx]=="TBAR-S-CH.root" ||samples_[idx]=="T-T-CH.root"||samples_[idx]=="WW.root" ||samples_[idx]=="WZ.root")) hList->Add(hists[idx]);
...
else {revDATAhists[3]->Fill( reader->EvaluateMVA( "BDT method"           ) );
...
datahists[3]->SetBinError(idx,(datahists[3]->GetBinContent(idx)/datahists[3]->Integral())*141.29);}
...
signalsamples_.push_back("SIGNALtGC_scaleup.root");
signalsamples_.push_back("SIGNALtGC_scaledown.root");
signalsamples_.push_back("SIGNALtGC_massup.root");
signalsamples_.push_back("SIGNALtGC_massdown.root");

float signalscales[] ={0.0922/19.145,0.0757/19.145,0.0950/19.145,0.10037/19.145};
...
if(sqrt(pow((*myetaphoton )[0]- 1.61,2)+pow((*myphiphoton )[0]+2.05,2))>0.05 && sqrt(pow((*myetaphoton )[0]-1.75,2)+pow((*myphiphoton )[0]-2.12,2))>0.05){
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup){
Shists[0] ->Fill( reader->EvaluateMVA( "BDT method"           ),finalweight*signalscales[phi]);
Shists[1] ->Fill( reader->EvaluateMVA( "BDT method"           ),finalweight*signalscales[phi]);
Shists[2] ->Fill( reader->EvaluateMVA( "BDT method"           ),finalweight*signalscales[phi]);
Shists[3] ->Fill( reader->EvaluateMVA( "BDT method"           ),finalweight*signalscales[phi]);
Shists[4] ->Fill( reader->EvaluateMVA( "BDT method"           ),finalweight*signalscales[phi]);
}
}}
}
...
/////////////////////////////////////////////////////////////////////////
TFile *target  = new TFile( "MVApptGc.root","RECREATE" );
  hList->Write();
   target->Close();
}
