...
using namespace std;
void BDToutputtopmasswindow3tGcsystematic() 
{   
#ifdef __CINT__
...
float scales[] = {0.628,0.0978,34.01,6.133,1.04,0.32,0.02,0.002,0.0961,0.0253,0.0224,0.0145,0.0125,0.0160,0.0158,0.0341,0.0341,0.0341,0.020,0.0017,0.0055,0.0032,0.00084,0.02,0.01139,0.01139,0.0612/19.145};
...
if (samples_[idx]=="SIGNALtGC.root")nsignalevent=nsignalevent+1;
...
cout<<"vecplus[p][m]"<<vecplus[p][m]<<endl;
cout<<"vecminus[p][m]"<<vecminus[p][m]<<endl;
}}
...
errorup[p]=pow(0.25*wjetandwphjet[0]->GetBinContent(p+1),2)+errorup[p];
errordown[p]=pow(0.25*wjetandwphjet[0]->GetBinContent(p+1),2)+errordown[p];
errorup[p]=pow(0.16*wjetandwphjet[1]->GetBinContent(p+1),2)+errorup[p];
errordown[p]=pow(0.16*wjetandwphjet[1]->GetBinContent(p+1),2)+errordown[p];
...
cout<<"errorup[p]"<<p<<errorup[p]<<endl;
cout<<"errordown[p]"<<p<<errordown[p]<<endl;
}
...
TLegend* leg = new TLegend(0.59,0.5,0.85,0.85);
  ...
  leg->AddEntry( hists[26], "Signal(tc#gamma) 1 pb"               , "L");
