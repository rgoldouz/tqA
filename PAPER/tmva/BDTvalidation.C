...
std::vector<TString> variables_;
  ...
  variables_.push_back("Deltaphiphotonmuon");
...
files.push_back(new TFile(samples_[idx], "update"));
}
...
//      TBranch *newBranch = theTree->Branch(newBDT, &new_v, newBDT+TString("/F"));
        TBranch *newBranch = theTree->Branch("new_v", &new_v, "new_v/F");
        ...
        theTree->Print();
	theTree->Write("analyzestep2/BDToutput");
//	files[idx]->Close();
	}
}
}
