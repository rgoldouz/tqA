...
void jetpt1( TString myMethodList = "" ) 
{   
...
hists[idx] ->Fill( (*myptjet)[0] ,finalweight*scales[idx] );
if (samples_[idx]=="WPHJET.root") insidewphjet=insidewphjet+finalweight;
}
else {
SBhists[idx] ->Fill( (*myptjet)[0] ,finalweight*scales[idx] );
...
else {revDATAhists[3]->Fill( (*myptjet)[0] );
...
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup){
Shists[0] ->Fill( (*myptjet)[0],finalweight*signalscales[phi]);
Shists[1] ->Fill((*myptjet)[0],finalweight*signalscales[phi]);
Shists[2] ->Fill( (*myptjet)[0],finalweight*signalscales[phi]);
Shists[3] ->Fill( (*myptjet)[0],finalweight*signalscales[phi]);
Shists[4] ->Fill( (*myptjet)[0],finalweight*signalscales[phi]);
}
}
...
/////////////////////////////////////////////////////////////////////////
TFile *target  = new TFile( "JETPT.root","RECREATE" );
  hList->Write();
   target->Close();
}
