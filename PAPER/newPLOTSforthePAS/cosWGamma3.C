...
void cosWGamma3() 
{   
  TString sysname ="COS_WGAMMA.root";
  TFile *sysinput(0);
...
hists[idx] ->Fill( (*mycoswphoton)[0],finalweight );
...
SBhists[idx] ->Fill( (*mycoswphoton)[0],finalweight );
...
//leptoncharge=(float)(*myleptoncharge )[0];
 if(sqrt(pow((*myetaphoton )[0]+1.76,2)+pow((*myphiphoton )[0]-1.37,2))>0.05 && sqrt(pow((*myetaphoton )[0]-2.37,2)+pow((*myphiphoton )[0]-2.69,2))>0.05){
if(sqrt(pow((*myetaphoton )[0]- 1.61,2)+pow((*myphiphoton )[0]+2.05,2))>0.05 && sqrt(pow((*myetaphoton )[0]-1.75,2)+pow((*myphiphoton )[0]-2.12,2))>0.05){
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup) datahists[idx] ->Fill( (*mycoswphoton)[0] );
else datahists[3]->Fill(  (*mycoswphoton)[0] );
}}}
delete myptphoton;
delete myetaphoton;
delete myptmuon;
...
hs1->GetXaxis()->SetTitle("Cos(W,#gamma)");
