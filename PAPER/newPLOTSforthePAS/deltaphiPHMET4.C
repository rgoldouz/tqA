...
void deltaphiPHMET4() 
{   
  TString sysname ="DELTAPHIPHMET.root";
  TFile *sysinput(0);
...
hists[idx] ->Fill((*mydeltaphiphotonmet)[0],finalweight );
...
SBhists[idx] ->Fill( (*mydeltaphiphotonmet)[0],finalweight );
...
revDATAhists[3]->Fill( (*mydeltaphiphotonmet)[0]);
}
...
hs1->GetYaxis()->SetTitle("Events / 0.32");
hs1->GetXaxis()->SetTitle("#Delta#phi (Photon,MET)");
...
h_ratio->GetXaxis()->SetTitle("#Delta#phi (Photon,MET)");
