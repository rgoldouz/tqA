...
void costopGamma3() 
{   
  TString sysname ="COS_TOPGAMMA.root";
  TFile *sysinput(0);
...
hists[idx] ->Fill( (*mycostopphoton)[0],finalweight );
...
SBhists[idx] ->Fill( (*mycostopphoton)[0],finalweight );
...
//leptoncharge=(float)(*myleptoncharge )[0];
 if(sqrt(pow((*myetaphoton )[0]+1.76,2)+pow((*myphiphoton )[0]-1.37,2))>0.05 && sqrt(pow((*myetaphoton )[0]-2.37,2)+pow((*myphiphoton )[0]-2.69,2))>0.05){
if(sqrt(pow((*myetaphoton )[0]- 1.61,2)+pow((*myphiphoton )[0]+2.05,2))>0.05 && sqrt(pow((*myetaphoton )[0]-1.75,2)+pow((*myphiphoton )[0]-2.12,2))>0.05){
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup) datahists[idx] ->Fill( (*mycostopphoton)[0] );
else datahists[3]->Fill(  (*mycostopphoton)[0] );
}}}
delete myptphoton;
delete myetaphoton;
delete myptmuon;
...
revDATAhists[idx]->Fill(  (*mycostopphoton)[0]);
}
else {
//revDATAhists[3]->Fill( reader->EvaluateMVA( "BDT method"           ) );
outsidewjet=outsidewjet+1;
revDATAhists[3]->Fill( (*mycostopphoton)[0]);
}}
}}
...
hs1->Draw("hist");
hs1->SetMaximum(1.4*datahists[2]->GetMaximum());
//hs1->GetXaxis()->SetTitle("BDT output");


hs1->GetYaxis()->SetTitle("Events / 0.07");
hs1->GetXaxis()->SetTitle("Cos(top,#gamma)");
...
h_ratio->GetXaxis()->SetTitle("Cos(top,#gamma)");
