...
void deltaphiPHMET1( TString myMethodList = "" ) 
{   
...
if (phi>5) finalweight=eventwight[systematics[phi].c_str()];
...
hists[idx] ->Fill( (*mydeltaphiphotonmet)[0] ,finalweight*scales[idx] );
if (samples_[idx]=="WPHJET.root") insidewphjet=insidewphjet+finalweight;
}
else {
SBhists[idx] ->Fill( (*mydeltaphiphotonmet)[0] ,finalweight*scales[idx] );
...
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup) datahists[idx] ->Fill( (*mydeltaphiphotonmet)[0]);
else datahists[3]->Fill( (*mydeltaphiphotonmet)[0] );
}
   // Get elapsed time
   sw.Stop();
   std::cout << "--- End of event loop: "; sw.Print();
...
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup) {
revDATAhists[idx]->Fill( (*mydeltaphiphotonmet)[0]);
insidewjet=insidewjet+1;}
else {revDATAhists[3]->Fill( (*mydeltaphiphotonmet)[0] );
...
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup){
Shists[0] ->Fill( (*mydeltaphiphotonmet)[0],finalweight*signalscales[phi]);
Shists[1] ->Fill((*mydeltaphiphotonmet)[0],finalweight*signalscales[phi]);
Shists[2] ->Fill( (*mydeltaphiphotonmet)[0],finalweight*signalscales[phi]);
Shists[3] ->Fill( (*mydeltaphiphotonmet)[0],finalweight*signalscales[phi]);
Shists[4] ->Fill( (*mydeltaphiphotonmet)[0],finalweight*signalscales[phi]);
}
}
...
/////////////////////////////////////////////////////////////////////////
TFile *target  = new TFile( "DELTAPHIPHMET.root","RECREATE" );
  hList->Write();
   target->Close();
}
