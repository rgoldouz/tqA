...
void Muonpt1( TString myMethodList = "" ) 
{   
...
hists[idx] ->Fill( (*myptmuon)[0] ,finalweight*scales[idx] );
if (samples_[idx]=="WPHJET.root") insidewphjet=insidewphjet+finalweight;
}
else {
SBhists[idx] ->Fill( (*myptmuon)[0] ,finalweight*scales[idx] );
...
else {revDATAhists[3]->Fill( (*myptmuon)[0] );
...
if(sqrt(pow((*myetaphoton )[0]- 1.61,2)+pow((*myphiphoton )[0]+2.05,2))>0.05 && sqrt(pow((*myetaphoton )[0]-1.75,2)+pow((*myphiphoton )[0]-2.12,2))>0.05){
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup){
Shists[0] ->Fill( (*myptmuon)[0],finalweight*signalscales[phi]);
Shists[1] ->Fill((*myptmuon)[0],finalweight*signalscales[phi]);
Shists[2] ->Fill( (*myptmuon)[0],finalweight*signalscales[phi]);
Shists[3] ->Fill( (*myptmuon)[0],finalweight*signalscales[phi]);
Shists[4] ->Fill( (*myptmuon)[0],finalweight*signalscales[phi]);
}}}
}
...
/////////////////////////////////////////////////////////////////////////
TFile *target  = new TFile( "MUONPT.root","RECREATE" );
  hList->Write();
   target->Close();
}
