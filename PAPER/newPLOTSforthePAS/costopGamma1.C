...
void costopGamma1( TString myMethodList = "" ) 
{   
...
hists[idx] ->Fill( (*mycostopphoton)[0] ,finalweight*scales[idx] );
if (samples_[idx]=="WPHJET.root") insidewphjet=insidewphjet+finalweight;
}
else {
SBhists[idx] ->Fill( (*mycostopphoton)[0] ,finalweight*scales[idx] );
...
else {revDATAhists[3]->Fill( (*mycostopphoton)[0] );
...
if((*mymasstop )[0]>mtopdown && (*mymasstop )[0]<mtopup){
Shists[0] ->Fill( (*mycostopphoton)[0],finalweight*signalscales[phi]);
Shists[1] ->Fill((*mycostopphoton)[0],finalweight*signalscales[phi]);
Shists[2] ->Fill( (*mycostopphoton)[0],finalweight*signalscales[phi]);
Shists[3] ->Fill( (*mycostopphoton)[0],finalweight*signalscales[phi]);
Shists[4] ->Fill( (*mycostopphoton)[0],finalweight*signalscales[phi]);
}
}
...
/////////////////////////////////////////////////////////////////////////
TFile *target  = new TFile( "COS_TOPGAMMA.root","RECREATE" );
  hList->Write();
   target->Close();
}
