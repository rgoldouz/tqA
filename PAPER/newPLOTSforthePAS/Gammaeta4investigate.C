...
cout<<"signal Integral   "<<hists[21+5]->Integral()<<endl;
  ...
  c35->cd(0);
datahists[2]->GetXaxis()->SetTitle("#eta (Signal Region)");
datahists[2]->GetYaxis()->SetTitle("NEVENTS");
datahists[2]->Draw();

  TCanvas *c36 = new TCanvas("c36","36",900,700);
  c36->cd(0);
revDATAhists[2]->GetXaxis()->SetTitle("#eta (Control Region)");
revDATAhists[2]->GetYaxis()->SetTitle("NEVENTS");
revDATAhists[2]->SetLineWidth(3.);
revDATAhists[2]->SetLineColor(kBlack);
revDATAhists[2]->SetMarkerColor(kBlack);
revDATAhists[2]->SetMarkerStyle(20.);
revDATAhists[2]->SetMarkerSize(1.35);
revDATAhists[2]->Draw();
